...
public static class ExpansionClaimsPrincipal
{
        ...
        string shema = CookieAuthenticationDefaults.AuthenticationScheme)
    {
        if (_userIssuer != null)
        {
            ...
            {
                claimsIdentity.AddClaim(new Claim(ClaimTypes.GroupSid, user.AccessLevelId.ToString()!,
                    nameof(Int32), _userIssuer));

                claimsIdentity.AddClaim(new Claim(ClaimTypes.PrimaryGroupSid, user.AccessLevel.Permission.ToString(),
                    nameof(Int32), _userIssuer));

                if (user.AccessLevel.Role != null)
                {
                    claimsIdentity.AddClaim(new Claim(ClaimTypes.Role, user.AccessLevel.Role.IdRole.ToString(),
                        nameof(Int32), _userIssuer));
                    claimsIdentity.AddClaim(new Claim(ClaimTypes.System,
                        user.AccessLevel.InformationSystem.IdInformationSystem.ToString(), nameof(Int32), _userIssuer));
                }
            }
    ...
    /// <returns></returns>
    public static async Task<bool> IsAuthUser<THub>(this AbstractController<THub, ServiceSystemContext> controller,
